...
## 16. Что такое HTML-форма?

HTML-форма (или веб-форма) – это раздел HTML-документа, предназначенный для сбора и отправки данных пользователя на сервер для обработки.

## 17. В чем разница между атрибутами «действие» и «метод» HTML-формы?

Атрибут action в теге `<form>` указывает куда отправлять данные формы (URL-адрес серверного скрипта), а атрибут method определяет как отправлять эти данные, используя один из HTTP-методов, например, GET или POST. POST отправляет данные в теле запроса и не имеет ограничений, а GET — в составе URL-адреса.

## 18. Что произойдет, если между тегами HTML нет текста?

Нечего было бы форматировать, если бы между тегами не было текста. А также такие теги создают пустой элемент, который может быть использован для структурных целей, например, для разделения содержимого, создания отступов или вставки элементов (картинок, ссылок).


 ## 19. Какова цель тега <head>?

Цель тега `<head>` — содержать служебную информацию (метаданные) о веб-странице, которая не отображается пользователю напрямую, но важна для браузеров и поисковых систем. Внутри `<head>` находятся такие элементы, как `<title>` (заголовок страницы в окне браузера), `<meta>` (описание страницы, ключевые слова), `<link>` (ссылки на стили CSS) и `<script>` (ссылки на скрипты JavaScript), которые помогают браузеру правильно отобразить страницу и улучшают её видимость в поисковых системах. 


## 20. Какова цель тега <meta> в HTML?

Цель тега `<meta>` в HTML — предоставление метаданных (структурированной вспомогательной информации) о веб-странице, которая не видна пользователям на странице, но важна для поисковых систем, браузеров. Эти данные помогают поисковикам индексировать страницу, а браузерам — правильно отображать и обрабатывать контент. 
